...
imsitu_dir = dataset_dir.joinpath('voa')
...
class ACE05FineTuneDataset(Dataset):
    def __init__(self, split='karpathy_train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train',collate_fn = None,dataset_prefix='2015',tokenizer = None):
        ...
        data_info_path = imsitu_dir.joinpath(f'voa_img.json') ## 修改
        ...
        for datum in imsitu_data:
            img_id = datum['img_id'].split('.jpg')[0]
        ...
        if self.args.max_n_boxes == 36:
            self.source_to_h5 = imsitu_dir.joinpath(f'voa_img/data_clip_{self.args.feature_type}_fc')
    ...
    def __getitem__(self, idx):
        ...
        if self.args.use_vision:
            ...
            # path = "/home/data2/datasets/swig_event/picture/data_clip_RN101_fc/loading_151.h5"
            with h5py.File(path, 'r') as f:
                # print(img_id)
                attr_feat = f[f"{img_id}/features"][...]
                fc_feat = torch.tensor(f[f"{img_id}/fc_features"][...]).unsqueeze(0)
                if f"{img_id}/regions" not in f:
                    print(img_id)
                    import pdb
                    pdb.set_trace()
        ...
        out_dict['target_ids'] = input_ids[10:]
        ...
        out_dict['target_length'] = len(input_ids[10:])
    ...
    def collate_fn(self, batch):
        ...
        if self.args.use_vision:
            ...
            feat_dim = batch[0]['fc_feats'].shape[-1]
        ...
        batch_entry['task'] = "pretrain"
        return batch_entry    
                           
def get_loader(args, split='train', mode='train',
               batch_size=32, workers=4, distributed=False, gpu=0,
               topk=-1,collate_fn=None,tokenizer = None, dataset_prefix = "swig"):
