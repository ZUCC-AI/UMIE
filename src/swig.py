...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None,  record_schema=None, decoding_format=None, decoding_type_schema=None,model_class=None, train=True, source_prefix=None):
        ...
        if 't5' in self.args.tokenizer or 'uie' in self.args.tokenizer : 
            ...
            for special_token in [constants.type_start, constants.type_end, constants.span_start, constants.spot_prompt, constants.asoc_prompt]+ [f'<vis_{i}>' for i in range(50-1, -1 , -1)]:
                if special_token not in self.tokenizer.get_vocab():
        ...
        from swig_clip_data import get_loader
    ...
    def train(self):
        if self.verbose:
            ...
            if not self.wandb_initialized:
                ...
                if 't5' in self.args.backbone:
                    project_name = "VLT5_SWiG"
                elif 'bart' in self.args.backbone:
                    project_name = "VLBART_SWiG"
                elif 'uie' in self.args.backbone:
                    project_name = "VLUIE_SWiG"
        ...
        for epoch in range(epochs):
            ...
            if self.verbose:
                ...
                val_results, predictions= self.evaluate(self.test_loader)

                # print(val_results)
                print(val_results)
                # wandb.log(val_results, step=epoch)
                # print(val_results)
                overall_F1 = val_results['overall-F1']
...
def main_worker(gpu, args):
    ...
    record_schema = RecordSchema.read_from_file("/home/data2/datasets/swig_event/record.schema")
    decoding_format = "tree"
    decoding_type_schema = "spotasoc"
    
    from swig_model import VLT5SWiG, VLT5SWiG

    if 't5' in args.backbone or 'uie' in args.backbone  :
        model_class = VLT5SWiG
