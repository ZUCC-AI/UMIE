...
ner_dir = dataset_dir.joinpath('MNRE')
  
class MNREFineTuneClipDataset(Dataset):
    def __init__(self, raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train',collate_fn = None,dataset_prefix='relation_mnre_v2',task="mnre",tokenizer = None):
        super().__init__()
        ...
        data_info_path = ner_dir.joinpath(f'{dataset_prefix}/{dataset_prefix}_{self.mode}_uie.json') ## 修改
        ...
        for datum in mner_data:            
            img_id = str(datum['image_id']).split('.')[0]
            new_datum = {
                'img_id': img_id,
                'sent': datum['text'].strip(),
                'targets':datum['record'].strip(),
                'h': datum['entity'][0]["text"],
                't': datum['entity'][1]["text"],
                'spot':datum['spot'],
                'asoc':datum['asoc'],
        ...
        if self.args.max_n_boxes == 36:
            self.source_to_h5 = ner_dir.joinpath(f'img_{dataset_prefix}/{self.mode}/data_clip_{self.args.feature_type}_fc')
    ...
    def __getitem__(self, idx):
        ...
        if self.args.no_prefix:
            ...
            input_tokens = [prefix] + [datum['t']] +['and'] + [datum['h']]
            # print(input_tokens)
            if self.args.oscar_tags:
...
def get_loader(args, split='train', mode='train',
               ...
               topk=-1,collate_fn=None, 
               tokenizer = None,
               dataset_prefix=None,task="mnre"):

    # if 'mscoco' in split:
    verbose = (gpu == 0)

    dataset = MNREFineTuneClipDataset(
        # raw_dataset=_dset,
        rank=gpu,
        topk=topk,
        verbose=verbose,
