...
split_text = re.compile(r"The entity type of \w+ is \w+")
...
def resplit_label_span(label, span, task="mner", split_symbol=span_start):
    ...
    if task == "mnre":
        splited_label_span = span.split(split_symbol)
        if len(splited_label_span) == 2:
            return (splited_label_span[0].strip(), splited_label_span[1].strip())
        else:
            return (label, span)

    if "The entity type of" in label_span:
        label = label_span.split()[-1].strip()
        span = " ".join(label_span.split()[4:-2]).strip()
        return label, span
    
    if "entity in the sentence is" in label_span:
        # import pdb
        # pdb.set_trace()
        label = label_span.split()[1].strip()
        st = label_span.index("is")
        span = " ".join(label_span.split()[7:]).strip()
        return label, span
    
    if split_symbol in label_span and task == "mner":
        splited_label_span = label_span.split(split_symbol)
        if len(splited_label_span) == 2:
            return splited_label_span[0].strip(), splited_label_span[1].strip()
...
def get_tree_str(tree):
    ...
    for element in tree:
        if isinstance(element, str):
            str_list += [element]
        if len(str_list) == 0 and isinstance(element, ParentedTree):
            str_list += [element.label()]
...
class EntityPredictParser(PredictParser):
...
class RelationPredictParser(PredictParser):
    ...
    def get_record_list(self, sel_tree, text=None):
        ...
        for spot_tree in sel_tree:
            ...
            r,t  = resplit_label_span(
                h, spot_text, task="mnre")
            if h is None or h == null_span:
                continue
            
            if t is None or t == null_span:
                continue
            # Drop empty generated type
            if r is None:
                continue
            # Drop invalid spot type
            if self.spot_set is not None and r not in self.spot_set:
                continue
...
class SWiGPredictParser(PredictParser):
    ...
    def decode(self, gold_list, pred_list, text_list=None, raw_list=None
        ...
        for gold, pred, text, raw_data in zip(gold_list, pred_list, text_list,
            ...
            # print(instance["pred_tree"], instance['pred_spot'])
        ...
        for spot_tree in sel_tree:
            ...
            for asoc_tree in spot_tree:
                ...
                asoc_label, asoc_text = resplit_label_span(
                    asoc_label, asoc_text, task='swig')
...
class EventPredictParser(PredictParser):
    ...
    def get_record_list(self, sel_tree, text=None):
        ...
        for spot_tree in sel_tree:
            ...
            spot_text = get_tree_str(spot_tree)
            spot_type, spot_text = resplit_label_span(
                spot_type, spot_text, task='event')
            spot_type, spot_text = rewrite_label_span(
                label=spot_type,
                span=spot_text,
                label_set=self.spot_set,
