...
imsitu_dir = dataset_dir.joinpath('m2e2')

class M2E2FineTuneDataset(Dataset):
    def __init__(self, split='karpathy_train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='test',collate_fn = None,dataset_prefix='2015',tokenizer = None,task="m2e2"):
        super().__init__()
        ...
        data_info_path = imsitu_dir.joinpath(f'm2e2_test_uie.json') ## 修改
        ...
        if self.args.max_n_boxes == 36:
            self.source_to_h5 = imsitu_dir.joinpath(f'm2e2/data_clip_{self.args.feature_type}_fc')
    ...
    def __getitem__(self, idx):
        ...
        if self.args.use_vision:
            ...
            with h5py.File(path, 'r') as f:
                ...
                assert len(vis_feats) == len(boxes), len(boxes)
        ...
        if self.args.no_prefix:
            ...
            if self.task.endswith("-arg") and datum['spot'] :
                prefix +=  datum['spot'][0]
    ...
    def collate_fn(self, batch):
        ...
        batch_entry['task'] = "m2e2"

        return batch_entry    
                           
def get_loader(args, split='train', mode='test',
               batch_size=32, workers=4, distributed=False, gpu=0,
               topk=-1,collate_fn=None,tokenizer = None, dataset_prefix = "m2e2", task="m2e2"):

    # if 'mscoco' in split:
    verbose = (gpu == 0)
    print(mode)
    dataset = M2E2FineTuneDataset(
        split,
        mode = mode,
        rank=gpu,
        topk=topk,
